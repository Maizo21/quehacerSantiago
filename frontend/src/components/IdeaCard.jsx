  ...
  return (
    <article className="group relative bg-card rounded-2xl border border-border overflow-hidden hover:border-sage/30 hover:shadow-[0_8px_30px_rgba(171,200,162,0.08)] hover:-translate-y-1 transition-all duration-300 flex flex-col">
      <Link href={`/idea/${idea.id}`} className="block relative h-52 overflow-hidden" aria-label={`Ver detalle de ${idea.titulo}`}>
        ...
        {idea.destacado && (
          <span className="absolute top-3 left-3 bg-sage text-dark text-xs font-bold px-2.5 py-1 rounded-full">
            Destacado
          </span>
        )}
        ...
        <div className="absolute top-3 right-3 flex gap-2 z-10">
          <button
            onClick={handleShare}
            className="bg-black/50 hover:bg-sage/80 text-light/80 hover:text-dark w-8 h-8 rounded-full flex items-center justify-center transition cursor-pointer backdrop-blur-sm"
          ...
          {isAdmin && (
            <button
              ...
              className="bg-black/50 hover:bg-red-900/80 text-light/70 hover:text-red-300 w-8 h-8 rounded-full flex items-center justify-center transition cursor-pointer backdrop-blur-sm"
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          <h3 className="font-bold text-white text-lg leading-snug line-clamp-2 drop-shadow-lg">
            {idea.titulo}
          </h3>
        </div>
      </Link>
          ...
          {visibleTags.map(tag => (
            <li key={tag}>
              <Link
                ...
                className={`text-xs px-2.5 py-1 rounded-full font-medium border hover:brightness-125 transition inline-block ${getTagStyle(tag)}`}
              >
                {tag}
              </Link>
            </li>
